...
function volcengineContentItem(inputKey: string, url: string): Record<string, unknown> | null {
  if (inputKey === 'volcengine_first_image_content') return { type: 'image_url', image_url: { url } }
  if (inputKey === 'volcengine_first_role_image_content') return { type: 'image_url', image_url: { url }, role: 'first_frame' }
  if (inputKey === 'volcengine_last_role_image_content') return { type: 'image_url', image_url: { url }, role: 'last_frame' }
  return null
}
    ...
    if (arr) {
      ...
      if (capped.length) {
        if (inputKey === 'volcengine_image_contents') {
          out[inputKey] = capped.map((url) => ({ type: 'image_url', image_url: { url } }))
