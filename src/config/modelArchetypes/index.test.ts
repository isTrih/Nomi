...
describe("resolveArchetypeForModel — 供应商无关的识别桥", () => {
  ...
  it("火山方舟 Seedance 2.0：标准/Fast 解析到火山专属档案，mini 不提前开放", () => {
    ...
    expect(arch.variants?.map((v) => v.id)).toEqual(["standard", "fast"]);
    expect(arch.variants?.some((v) => v.id === "mini")).toBe(false);
  });

  it("火山方舟 Seedance Fast 变体：resolution 收窄到 480/720", () => {
