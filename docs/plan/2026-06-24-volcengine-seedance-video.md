...
- 不改画布 UI 布局和设计系统。
- 不提前开放文档标注 2026-06-25 才预计支持 API 的 Seedance 2.0 mini。
...
- 新增 `src/config/modelArchetypes/seedanceVolcengine.ts`：声明火山 Seedance 2.0 档案和标准/Fast 变体。
- 在 `seedBuiltins` 和 `MODEL_ARCHETYPES` 注册。
- 补单测锁住请求体、任务轮询响应和 seed 对账。

## 验收

- 火山方舟模型目录出现 `doubao-seedance-2-0-260128`，模型档案为 `volcengine-seedance-2`。
